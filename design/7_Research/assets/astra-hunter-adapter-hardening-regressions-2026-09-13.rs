//! Preserved public-API diagnostics adapted for a559e54's explicit events/Result API.
//! Original f92e669 fixture remains unchanged. Geometry now must be explicitly rejected.
#![cfg(test)]
...
fn a_supported_named_rig_agrees_with_admitted_core_contact_geometry() {
    ...
    let mut p = presenter();
    p.observe(&view);
    let before = cubarium_core::snapshot::state_hash(&w.state);
    assert!(p.validate_hunter_profile(w.hunters().profile().unwrap()).is_err());
    let err = p.observe_hunters(&view, &[h], &[]).unwrap_err();
    assert!(err.contains("capture offset"));
    assert!(!p.hunter_ids().contains(&h.id));
    assert_eq!(cubarium_core::snapshot::state_hash(&w.state), before);
}

#[test]
fn every_admitted_profile_scale_matches_the_named_contact_effectors() {
    use cubarium::hunter_present::{validate_profile, validate_view};
    use cubarium_core::hunter::{body_scale, ContactGeometry};
    let w = world(false);
    for minimum in [0.2, 0.316, 0.632, 1.0] {
        for exponent in [0.0, 0.25, 0.5, 1.0, 100.0] {
            let mut profile = w.hunters().profile().unwrap().clone();
            profile.body_scale_min = minimum;
            profile.body_scale_exponent = exponent;
            validate_profile(&profile).unwrap();
            for ratio in [0.0, 1e-12, 0.01, 0.1, 0.5, 0.999999, 1.0, 1.01] {
                let mut h = w.hunter_view()[0];
                h.body_scale = body_scale(&profile, ratio, 1.0);
                let mut organism = w.state.organisms.get(h.id).unwrap().clone();
                organism.structure = ratio;
                organism.phenotype.structure_adult = 1.0;
                h.geometry = ContactGeometry::of(&profile, &organism);
                validate_view(&h).unwrap();
                let effectors = lanternjaw::effectors(h.body_scale);
                assert!((h.geometry.capture_offset_body - effectors.near_claw).length() < 1e-9);
                assert!((h.geometry.ingestion_offset_body - effectors.mouth).length() < 1e-9);
            }
        }
    }
}
