//! Two art packs, one real world, the same actual meal controller enabled in both.
use cubarium::{art::ArtPack, art_present::{ArtPresenter, rig_of}, sink::png::write_net_png};
use cubarium_core::{World, decode_snapshot, care::{CareCommand, CareKind, CareTarget}};
    ...
    for elapsed in 0..1000u64 {
        if elapsed == 600 {
            receipt = serde_json::to_value(world.apply_care(&CareCommand::standard(
                world.care().admitted_seq + 1, world.tick(), CareKind::Feed,
                CareTarget { face: 0, u: 32., v: 48. },
            ))).unwrap();
        }
        world.step();
        world.drain_events();
        ...
        for o in &view.organisms {
            if rig_of(o.form, o.hue, 4) == 1 && o.fed { fed_sail_ticks += 1; }
            if (590..=900).contains(&elapsed) && rig_of(o.form, o.hue, 4) == 1 {
                let memory = new.meal_of(o.id).map(|m| json!({"onset":m.onset,"weight":m.weight,"weight_prev":m.weight_prev}));
        ...
        if (590..=900).contains(&elapsed) {
            ...
            for k in 0..3u64 {
                for (name, presenter) in [("old", &mut old), ("new", &mut new)] {
                    ...
                    write_net_png(&out.join(name).join(format!("frame_{:05}.png", elapsed * 3 + k)), &rgb).unwrap();
                }
                frame_pairs += 1;
            }
            assert_eq!(hash, cubarium_core::snapshot::state_hash(&world.state), "drawing mutated ecology");
        }
    }
