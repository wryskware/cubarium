...
## Original shipped atlas versus the explicit stable-body + fin4 source candidate.
## Run before replacing the production atlas; baseline is read, never regenerated.
...
func study() -> void:
	...
	if failed:
		quit(1)
		return
	var shipped := Image.load_from_file(baseline_path)
	...
	for mode_index in range(MODES.size()):
		...
		var timing := [0, 0]
		var new_point := Image.create(TILE * FRAMES, TILE, false, Image.FORMAT_RGBA8)
		for frame in range(FRAMES):
			select_pose(player, mode, float(frame) / (FRAMES - 1 if mode == "bud" else FRAMES) * clip.length)
			var began := Time.get_ticks_usec()
			var point := raster_point(rig)
			timing[0] += Time.get_ticks_usec() - began
			began = Time.get_ticks_usec()
			var fin4 := raster(rig)
			timing[1] += Time.get_ticks_usec() - began
			baseline.blit_rect(shipped, Rect2i(frame * TILE, (4 + mode_index) * TILE, TILE, TILE), Vector2i(frame * TILE, 0))
			new_point.blit_rect(point, Rect2i(0, 0, TILE, TILE), Vector2i(frame * TILE, 0))
			...
			var alpha_sum := 0.0
			if not same_visible(crisp, raster(rig)): fail(mode + ": non-fin layer changed")
			for y in range(TILE):
				for x in range(TILE):
