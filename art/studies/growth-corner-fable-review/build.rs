...
// the real private `draw_column` from the host source, plus a copy whose near-corner cap
// owner is held at the column's final cap position. Reproduced here so the cost probe
// times exactly what the study draws; production visibility is not changed.
fn main() {
